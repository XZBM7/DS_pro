...
def save_prediction():
    try:
        ...
        # Map frontend fields to dataset columns when saving
        doc = {
            "user_id": user_id,
        ...
        # Include the 10 chosen input fields (if present) in the doc for traceability
        for friendly, ds_col in friendly_to_dataset.items():
            val = fields.get(friendly, None)
            doc[friendly] = val
        
        # also store some dataset column defaults for context
        # (we avoid saving the entire dataset row to keep doc small)
...
def get_records():
    ...
    for d in coll.find({"user_id": user_id}).sort("created_at", -1):
        rows.append({
            "_id": str(d.get("_id")),
            # include both friendly fields and pred/level
            "pred": d.get("pred"),
            "level": d.get("level"),
...
def export_csv():
    ...
    # header includes friendly fields + pred/level
...
def api_dataset_stats():
    try:
        correlations = {}
        # correlation examples with previous scores and hours studied
        ...
        # study hours impact (Hours_Studied)
        study_time_impact = {}
        if 'Hours_Studied' in df.columns and 'Exam_Score' in df.columns:
...
def api_feature_importance():
    try:
        ...
        # Aggregate by original feature where possible
        original_feature_importance = {}
        
        for i, feature in enumerate(feature_names):
            original_feature = feature
            # best-effort mapping: remove transformer prefixes if present
            original_feature = re.sub(r'^(num__|cat__)', '', original_feature)
            # collapse one-hot suffixes
